...
int main(int argc, char **argv)
{
    ...
    constexpr size_t c_pattern_counts[] = { 64, 256, 1024, 4096, 16384, 65536, 262144, 1048576 };
    constexpr size_t c_scale_count = sizeof(c_pattern_counts) / sizeof(c_pattern_counts[0]);
    ...
    size_t const output_byte_count = c_pattern_counts[c_scale_count - 1] * pattern_byte_count;
    ...
    for (size_t count : c_pattern_counts) {
        if (write_csv)
        ...
        for (size_t fi = 0; test_func_t func : c_test_funcs) {
            ...
            do {
                rt.BeginTimeBlock();
                {
                    for (size_t i = 0, end = output_byte_count / (pattern_byte_count * count); i < end; ++i) {
                        char *dst = output_mem;
                        for (size_t j = 0; j < count; ++j, dst += pattern_byte_count)
            ...
            if (write_csv) {
                printf(",%Lf", gb_per_measure(
                    (long double)results.min_ticks / cpu_timer_freq, output_byte_count));
            } else {
                char namebuf[256];
                snprintf(
